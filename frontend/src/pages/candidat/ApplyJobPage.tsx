...
import { AxiosError } from 'axios';  // ✅ Pour typer les erreurs API
...
const onFinish = async (values: ApplicationFormValues) => {  // ✅ Typé
  setLoading(true);
  try {
    ...
    // Sérialisation avec vérification
    const serializeIfNotEmpty = (key: string, data: unknown) => {
      if (Array.isArray(data) && data.length > 0) {
        formData.append(key, JSON.stringify(data));
      }
    };
    
    serializeIfNotEmpty('experiences', values.experiences);
    serializeIfNotEmpty('formations', values.formations);
    serializeIfNotEmpty('skills', values.skills);
    serializeIfNotEmpty('challenges', values.challenges);

    // Upload CV avec vérification de type
    const cvFile = values.cv?.fileList?.[0]?.originFileObj;
    ...
    // Gestion type-safe avec AxiosError
