...
const RegisterManager: React.FC = () => {
  ...
  const onFinish = async (values: any) => {
    setLoading(true);
    try {
      // Suppression de l'apostrophe problématique dans la requête
      ...
      message.success('Inscription Manager réussie ! Votre compte est en attente d\'approbation.');
      navigate('/login/manager');
    } catch (error: any) {
      message.error(error.message || 'Erreur lors de l\'inscription Manager');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #f0f9ff 0%, #e6f7ff 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
      <Card style={{ width: '100%', maxWidth: '480px', borderRadius: '16px', boxShadow: '0 12px 30px rgba(0,0,0,0.12)' }}>
        <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
          <Title level={2} style={{ margin: 0, color: '#00a89c' }}>Talentflow - Manager</Title>
          <Text type="secondary">Inscription Manager Talentflow</Text>
          <Alert 
        ...
        <Form layout="vertical" onFinish={onFinish} autoComplete="off">
          ...
          <Form.Item 
            name="position" 
            rules={[{ required: true, message: 'Position hiérarchique obligatoire' }]}
            initialValue="Chef de departement"
          >
            <Select placeholder="Position hierarchique" size="large" style={{ borderRadius: '8px' }}>
              <Option value="Chef de departement">Chef de departement</Option>
              <Option value="Directeur">Directeur</Option>
              <Option value="Responsable equipe">Responsable equipe</Option>
            </Select>
          </Form.Item>
          ...
          <Form.Item>
            ...
            >
              Soumettre ma candidature Manager
            </Button>
          </Form.Item>
        </Form>
