    ...
    >
      ...
      {loading && jobs.length === 0 ? (
        ...
        <Card style={{ textAlign: 'center', padding: 60 }}>
          ...
          {!searchQuery && (
            <Button type="primary" icon={<PlusOutlined />} onClick={() => navigate('/jobs?action=create')}>
        ...
        <List
          ...
          renderItem={(job: Job) => {
            ...
            return (
              <List.Item
                actions={[
                  <Button key="edit" type="text" icon={<EditOutlined />} size="small" onClick={() => jobModal.open(job)}>
                    Modifier
                  </Button>,
                  <Button 
                    key="delete" 
                ...
                style={{ padding: '16px 24px', borderBottom: '1px solid #f0f0f0', cursor: 'pointer' }}
                  ...
                  description={
                    <Space direction="vertical" size={8} style={{ marginTop: 8 }}>
                      <Space size={24} style={{ fontSize: 13 }}>
                        ...
                        <Text type="secondary">📍 {job.type_lieu === 'remote' ? '🏠 Remote' : job.type_lieu === 'hybrid' ? '🔄 Hybride' : '🏢 Sur site'}</Text>
                      </Space>
                      <Space size={24} style={{ fontSize: 13 }}>
                        <Text type="secondary">📩 {job.applications_count ?? 0} candidature{job.applications_count !== 1 ? 's' : ''}</Text>
                        {job.date_limite && <Text type="secondary">📅 {new Date(job.date_limite).toLocaleDateString('fr-FR')}</Text>}
                        {job.salaire_min && job.salaire_max && <Text type="secondary">💰 {job.salaire_min} - {job.salaire_max} TND</Text>}
                      </Space>
                      {job.description && (
                        <Text type="secondary" style={{ fontSize: 13, maxWidth: 800, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                          {job.description}
                        </Text>
                      )}
                    </Space>
