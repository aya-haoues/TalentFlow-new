...
export const authService = {
  ...
  login: async (
    credentials:
      | LoginFormData
      | { provider: "google" | "linkedin"; token: string },
  ...
  getCurrentUserFromServer: async (): Promise<User | null> => {
    try {
      ...
      if (user?.id) {
        // ✅ vérifier que c'est bien un user avant de stocker
...
// ============================================================================
// 👨‍💼 SERVICE MANAGER / RECRUTEUR TECHNIQUE
// ============================================================================
export const managerService = {
  ...
  submitEvaluation: async (id: string | number, data: any) => {
    const response = await api.post(
      `/manager/applications/${id}/evaluate`,
      data
    );
    return response.data;
  },
